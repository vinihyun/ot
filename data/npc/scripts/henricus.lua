...
local function creatureSayCallback(cid, type, msg)
	...
	elseif(msgcontains(msg, "mission") or msgcontains(msg, "report")) then
		if(player:getStorageValue(Storage.TheInquisition.Questline) == 1) then
			npcHandler:say({"Let's see if you are worthy. Take an inquisitor's field guide from the box in the back room. ...","Follow the instructions in the guide to talk to the Hammerfell guards that protect the walls and gates of the city and test their loyalty. Then report to me about your {mission}."}, cid, 0, 1, 3000)
			npcHandler.topic[cid] = 0
		...
		elseif(player:getStorageValue(Storage.TheInquisition.Questline) == 11) then
			npcHandler:say({"We've got a report about an abandoned and haunted dirty house in Hammerfell. I want you to examine this house. It's the only ruin in Hammerfell so you should have no trouble finding it. ...","There's an evil being somewhere. I assume that it will be easier to find the right spot at night. Use this vial of holy water on that spot to drive out the evil being."}, cid, 0, 1, 3500)
		...
		elseif(player:getStorageValue(Storage.TheInquisition.Questline) == 12 or player:getStorageValue(Storage.TheInquisition.Questline) == 13) then
			npcHandler:say("Your current mission is to exorcise an evil being from a house in Hammerfell. Are you done with that mission? ", cid)
		...
		elseif(player:getStorageValue(Storage.TheInquisition.Questline) == 17) then
			npcHandler:say({"We've got information about something very dangerous going on in the Sinsville. The demons are preparing something there ...","Something that is a threat to all of us. Our investigators were able to acquire vital information before some of them were slain by a demon named Ungreez. ...","It'll be your task to take revenge and to kill that demon. You'll find him in one of the crypts in Sinsville. Good luck."}, cid, 0, 1, 3200)
			npcHandler.topic[cid] = 0
